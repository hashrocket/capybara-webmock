    ...
    describe '#start' do
      it 'starts a process' do
        ...
        expect(Open3).to have_received(:popen2e).with(
          { "PROXY_PORT_NUMBER" => "9292", "__CAPYBARA_WEBMOCK_ADDED_HOSTS" => "" },
          "rackup",
      ...
      it 'uses a custom port' do
        ...
        Capybara::Webmock.start
        expect(Open3).to have_received(:popen2e).with({ "PROXY_PORT_NUMBER" => "8873", "__CAPYBARA_WEBMOCK_ADDED_HOSTS" => "" }, anything, anything)
      end

      it 'uses extra hosts' do
        Capybara::Webmock.allowed_hosts = %w[sub.example.com example.com]
        Capybara::Webmock.start
        expect(Open3).to have_received(:popen2e).with({ "PROXY_PORT_NUMBER" => "9292", "__CAPYBARA_WEBMOCK_ADDED_HOSTS" => "sub.example.com|example.com" }, anything, anything)
      end
    end
